...
# from fit01_steps-1k.py
# - tgt_ages: [7, 52] -> [5, 7, 24, 52, 72]
# - fit.em.n_steps: 1000 -> 2000
# - project_dir -> fit02_ages-all
# from fit02_ages-all.py
# - exclude_keypoints: -> [t_tip]
# - project_dir -> fit03_kp-all
# from fit03-0_kp-all.py
# - morph.dist_var: [1e-1, 8e-1, 3e2] -> [5e-2, 2e-1, 4e-1, 8e-1] 
# - pose.calibrate.n_components -> 8
# from fit03-3_kp-all_strict-8comp
# - morph.dist_var: [5e-2, 2e-1, 4e-1, 8e-1] -> [1e-3, 1e-2, 5e-2, 4e-1]
# from fit03-7_kp-all_stricter-8comp
# - pose.calibration.max_components: 10 -> 20
# - pose.calibration.mode = "likelihood"
# - pose.calibration.sessions = "all"
# - pose.calibrate.n_components: 8 -> None
# from fit04-7-many-frames_likeligmm
# - fit.n_steps: 2000 -> 4000
# - lr.factor: 2 -> 1.25
# from fit04-8_longfit
# - animals = ['0', '1', '2', '3']
# from fit05-0_all-animals
# - features.calibration.tgt_variance = 0.95
# from fit05-1_lowpc
# - morph.dist_var -> [logspace -5...3 (n=15)]
...
fname = Path(__file__).name.split('_')[0]
project_dir = Path(
    f"/n/groups/datta/kai/mph/testsets/blscale-script-projects/{fname}_many-stiffs"
)
...
cfg["features"]["calibration"]["tgt_variance"] = 0.95
...
scan_cfg, model_cfg = scans.setup_scan_config(
    ...
    "morph-prior-scan",
    {
        "morph.dist_var": np.logspace(-5, 3, 15).tolist(),
        "fit.em.learning_rate.lr": [8e-1] * 15,
        "fit.em.n_steps": [4000] * 15,
    },
)
